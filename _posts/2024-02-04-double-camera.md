...
そういえば同じパターンの繰り返ししかなかったんでした。
しかしわずかな希望があります。1フレームの最後のパケットには `0xFF 0x09` があります。
...
ここに至って最後の仮説が出てきます。
すなわち、パケットのデータは**量子化テーブルやハフマンテーブルを除いた画像データのみ**なのではないか？
...
JPEG実装の手習として、[libKPEG](https://github.com/TheIllusionistMirage/libKPEG)というものが公開されていたので中を覗いてみると、勧告T.81と同じテーブルを用いてJPEGファイルを作成するコードがありました。
...
まず、専用アプリが入ったiPadとMacを接続し、端末のバックアップをとります。すると `~/Library/Application Support/MobileSync/Backup/(長い識別子)/` にバックアップが保存されます。
...
`Manifest.db` はSQLite3 のデータベースなので、`sqlite3` コマンドで開きます。
このDBファイルのテーブル一覧を `.schema` コマンドで表示するとこうなります。
...
`Files` テーブルの `fileID` がMacのバックアップでのファイルIDで、
...
ファイルIDは `83cd78fbdc6f53c56e363a745df05a733bd7e110` でした。
そしてバックアップフォルダの中に `83/83cd78fbdc6f53c56e363a745df05a733bd7e110` という場所に見合ったため、拡張子 `.mov` を付与して開いてみます。
